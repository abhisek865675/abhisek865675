# Hi there, I'm Abhisek! 👋

### 🛡️ Cybersecurity Trainer | Certified Ethical Hacker (CEH) | Penetration Tester

Welcome to my GitHub! I am a passionate cybersecurity professional and educator currently leading CEH classes. My approach to infosec is deeply rooted in **First Principles Thinking**—breaking down complex systems and protocols (like NAT, ARP, and SQL) to visualize their underlying mechanics before attempting to exploit them. 

---

## 👨‍💻 About Me

* 🎓 Currently working as a **Professional Cybersecurity Trainer**, guiding trainees through the complexities of ethical hacking and real-world security scenarios.
* 🎥 Creator of **Script to Root**, a YouTube channel dedicated to documenting my learning journey and breaking down complex infosec concepts for beginners. 
* ☁️ Regularly managing and experimenting within cloud environments, specifically building out **AWS Ubuntu EC2** instances for technical labs.
* 🎯 Always hunting for the next challenge in Exploit Development, Vulnerability Assessment, and Advanced Privilege Escalation.

---

## 🛠️ Technical Skills & Arsenal

### **Offensive Security & Concepts**
* Web & Network Penetration Testing
* Vulnerability Assessment & Mitigation
* Exploit Development & Privilege Escalation

### **Tools & Cloud**
* Burp Suite Pro, Metasploit Framework
* AWS (Cloud Infrastructure & EC2 Management)

### **Vulnerabilities Mastered**
* SQL Injection (SQLi)
* Cross-Site Scripting (XSS)
* XML External Entity (XXE)

---

## 🎯 Training Grounds & Platforms

I actively hone my skills and maintain my edge on industry-leading platforms:
* **PortSwigger Academy**: Deep diving into advanced Web Pentesting techniques.
* **Hack The Box (HTB) & TryHackMe (THM)**: Regular lab solver. 
  * *Notable HTB Machines Owned:* Meow, Redeemer, Archetype, Three, Responder, Oopsie, and Dancing.

---

## 📈 GitHub Stats

![Abhisek's GitHub stats](https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&show_icons=true&theme=radium)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_GITHUB_USERNAME&layout=compact&theme=radium)

*(Make sure to replace `YOUR_GITHUB_USERNAME` in the image links above with your actual GitHub username!)*

---

## 📫 Let's Connect

* 💼 **LinkedIn:** [Abhisek on LinkedIn](#) *(Add your profile link here)*
* 📧 **Email:** [Contact Me](abhisehekrana53184@gmail.com)

> *"Understanding the absolute mechanics of a system is the first step to dismantling its security."*
